...
import { Save, User as UserIcon, Building, Palette, Shield, Layout, AlertTriangle, RefreshCw, Image as ImageIcon, GripVertical, Trash2 } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'company' | 'team' | 'workflow' | 'appearance' | 'login' | 'security'>('company');
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 gap-6 max-w-xl">
                <div>
                  ...
                  <input type="text" value={localSettings.companyName} onChange={(e) => setLocalSettings({...localSettings, companyName: e.target.value})} className="w-full p-2 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                   ...
                   <div className="flex gap-4 items-center">
                      ...
                      <input type="text" value={localSettings.companyLogo} onChange={(e) => setLocalSettings({...localSettings, companyLogo: e.target.value})} placeholder="https://..." className="flex-1 p-2 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:ring-2 focus:ring-indigo-500" />
                   </div>
                </div>
              </div>
            </div>
               ...
               <div className="space-y-3 max-w-2xl">
                   {localWorkflow.map((stage, index) => (
                       <div key={stage.id} className="flex items-center gap-3 p-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg group">
                           ...
                           <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-300">{index + 1}</div>
                           <input type="text" value={stage.name} readOnly className="flex-1 font-medium text-gray-800 dark:text-white focus:outline-none bg-transparent" />
                   ...
                   <div className="flex items-center gap-3 p-3 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-lg mt-4">
                       ...
                       <input type="text" value={newStage} onChange={(e) => setNewStage(e.target.value)} placeholder="Enter new stage name..." className="flex-1 bg-transparent focus:outline-none text-sm dark:text-white" onKeyDown={(e) => e.key === 'Enter' && handleAddStage()} />
             ...
             <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-4">
                        {colors.map(color => (
                            <button key={color.name} onClick={() => setLocalSettings({...localSettings, themeColor: color.name as any})} className={`w-16 h-16 rounded-xl ${color.hex} flex items-center justify-center transition-transform hover:scale-105 ${localSettings.themeColor === color.name ? 'ring-4 ring-offset-2 ring-gray-200 dark:ring-slate-700 scale-105' : ''}`}>
             ...
             <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 gap-6 max-w-xl">
                    <div>
                        ...
                        <input type="text" value={localSettings.loginScreen.title} onChange={(e) => setLocalSettings({...localSettings, loginScreen: { ...localSettings.loginScreen, title: e.target.value }})} className="w-full p-2 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:ring-2 focus:ring-indigo-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Subtitle</label>
                        <input type="text" value={localSettings.loginScreen.subtitle} onChange={(e) => setLocalSettings({...localSettings, loginScreen: { ...localSettings.loginScreen, subtitle: e.target.value }})} className="w-full p-2 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:ring-2 focus:ring-indigo-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Banner Image URL</label>
                        <input type="text" value={localSettings.loginScreen.bannerUrl} onChange={(e) => setLocalSettings({...localSettings, loginScreen: { ...localSettings.loginScreen, bannerUrl: e.target.value }})} placeholder="https://..." className="w-full p-2 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg focus:ring-2 focus:ring-indigo-500" />
                    </div>
                </div>
             </div>
          )}
             ...
             <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="mt-8 pt-6 border-t border-red-100 dark:border-red-900/30">
                     ...
                     <div className="flex items-center justify-between p-4 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30">
                        ...
                        <button onClick={() => { if(confirm('Are you sure? This will wipe all your changes.')) onResetApp(); }} className="px-4 py-2 bg-white dark:bg-slate-800 border border-red-200 dark:border-red-800 text-red-600 text-sm font-bold rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors flex items-center gap-2">

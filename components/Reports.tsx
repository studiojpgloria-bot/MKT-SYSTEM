...
export const Reports: React.FC<{ tasks: Task[]; users: User[]; workflow: WorkflowStage[]; themeColor: string; }> = ({ tasks, users, workflow, themeColor }) => {
  ...
  const reportData = useMemo(() => {
    ...
    const memberProductivity = users.map(user => {
        ...
        return { name: user.name, total: userTasks.length, completed: userCompleted, time: totalTime / 60 };
    ...
    return {
      totalTasks, completedTasks, approvalRate, avgTimeDays, activeMembers, overdueTasks,
      memberProductivity, tagDistributionData, creationTrendData, stageDistributionData
    };
  }, [tasks, users, workflow]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <KPICard label="Total Tasks" value={reportData.totalTasks} icon={FileText} colorClass="blue" />
          <KPICard label="Completed" value={reportData.completedTasks} icon={CheckSquare} colorClass="emerald" />
          <KPICard label="Approval Rate" value={`${reportData.approvalRate}%`} icon={TrendingUp} colorClass="violet" />
          <KPICard label="Avg Time/Task" value={`${reportData.avgTimeDays}d`} icon={Clock} colorClass="orange" />
          <KPICard label="Active Members" value={reportData.activeMembers} icon={Users} colorClass="rose" />
          <KPICard label="Overdue" value={reportData.overdueTasks} icon={BarChart2} colorClass="amber" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
              ...
              <div className="space-y-4">
                  {reportData.memberProductivity.map((member) => (
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
              ...
              <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={reportData.stageDistributionData} layout="vertical" margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
                          ...
                          <Bar dataKey="tasks" fill={getThemeColorHex()} radius={[0, 4, 4, 0]} barSize={20}>
                              {reportData.stageDistributionData.map((entry, index) => (
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 lg:col-span-2">
              ...
              <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={reportData.creationTrendData}>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
              ...
              <div className="h-64 flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                          <Pie data={reportData.tagDistributionData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                              {reportData.tagDistributionData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                          </Pie>
                          <Tooltip contentStyle={{ borderRadius: '8px', border: 'none' }} />
                      </PieChart>
                  </ResponsiveContainer>
              ...
              <div className="grid grid-cols-2 gap-y-2 gap-x-4 mt-2">
                  {reportData.tagDistributionData.map((item) => (

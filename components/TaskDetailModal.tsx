...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ 
  ...
  // Identifica se é uma tarefa nova que ainda não existe na lista global
  const isDraft = !allTasks.some(t => t.id === task?.id);
  const isAssignee = editedTask?.assigneeId === currentUser.id;
  const isAdminOrManager = currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.MANAGER;
  
  // BLOCO CORRIGIDO: Só mostra aprovação se a tarefa for REAL e o estágio for approved
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 dark:bg-black/90 backdrop-blur-md p-4 overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="flex-1 p-10 pt-0 overflow-y-auto custom-scrollbar space-y-10">
            ...
            {showAcceptPrompt && (
                <div className="bg-indigo-500/5 dark:bg-indigo-500/10 border border-indigo-500/10 dark:border-indigo-500/20 rounded-[32px] p-8 flex flex-col items-center justify-center text-center space-y-4 animate-in fade-in zoom-in duration-300 h-[400px]">
                    ...
                    <button onClick={() => onAccept(editedTask.id)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-12 py-5 rounded-2xl font-black shadow-xl shadow-indigo-600/20 text-xs uppercase tracking-widest">ACEITAR TAREFA AGORA</button>
                </div>
            )}

            {/* ENTREGA FINAL: Só aparece se aprovado e se não for rascunho */}
            {isApproved && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-indigo-600/5 dark:bg-indigo-500/10 border border-indigo-500/10 dark:border-indigo-500/20 rounded-[32px] p-8 relative overflow-hidden">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                            ...
                            {isAdminOrManager ? (
                                <div className="flex gap-2 w-full md:w-auto">
                                    ...
                                    <button onClick={() => handleSaveField('finalLink', tempFinalLink)} className="p-4 bg-indigo-600 text-white rounded-2xl"><CheckCircle size={20}/></button>
                                </div>
                            ) : editedTask.finalLink && (
                                <a href={editedTask.finalLink} target="_blank" rel="noreferrer" className="px-10 py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl flex items-center gap-3">
            ...
            <div className="space-y-4">
              ...
              <textarea 
                ...
                className="w-full p-6 rounded-[32px] border border-slate-200 dark:border-[#2a303c] bg-slate-50/50 dark:bg-[#151a21]/30 text-slate-700 dark:text-gray-300 focus:ring-1 focus:ring-indigo-500 resize-none h-48 placeholder-slate-300 dark:placeholder-gray-800"
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                    {(editedTask.subtasks || []).map(st => (
                        <div key={st.id} className="flex items-center gap-4 bg-slate-50/50 dark:bg-[#151a21]/20 p-4 rounded-2xl border border-transparent hover:border-slate-200 dark:hover:border-[#2a303c]">
                            <button onClick={() => handleSaveField('subtasks', editedTask.subtasks.map(s => s.id === st.id ? {...s, completed: !s.completed} : s))} className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${st.completed ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 dark:border-[#2a303c]'}`}>{st.completed && <CheckCircle size={14} className="text-white" />}</button>
                    ...
                    <input type="text" value={newSubtask} onChange={(e) => setNewSubtask(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSubtask()} placeholder="Adicionar item ao checklist..." className="w-full bg-transparent border-dashed border-2 border-slate-200 dark:border-[#2a303c] rounded-2xl p-4 text-sm font-bold text-slate-500" />
                </div>
            </div>
                    ...
                    {editedTask.comments?.map(c => {
                        ...
                        return (
                            <div key={c.id} className="flex gap-4">
                                <img src={commenter?.avatar} className="w-10 h-10 rounded-full border border-slate-200 dark:border-[#2a303c]" />
          ...
          <div className="w-full md:w-[400px] p-10 space-y-10 bg-slate-50 dark:bg-[#0b0e11] border-l border-slate-200 dark:border-[#2a303c] transition-colors duration-300">
             ...
             <div className="mt-auto pt-10 flex gap-4">
                 <button onClick={onClose} className="flex-1 py-5 text-xs font-black text-slate-400 border border-slate-200 dark:border-[#2a303c] rounded-2xl hover:bg-slate-100 dark:hover:bg-white/5 uppercase tracking-widest">Sair</button>
                 {isDraft && onCreate && (
                   <button onClick={() => onCreate(editedTask)} className="flex-[2] py-5 text-xs font-black text-white bg-green-600 rounded-2xl shadow-lg shadow-green-500/20 uppercase tracking-widest hover:scale-105 active:scale-95 transition-all">Criar Projeto</button>
                 )}
             </div>
          </div>
        </div>
